						...
						<table style="width: 100%">
							...
							<c:forEach items="${ram}" var="ram" varStatus="loopStatus">
								<tr bgcolor="${loopStatus.index % 2 == 0 ? '' : '#F0F0F0'}">
									<td><h5>${ram.modelo}</h5></td>
									<td><h5>${ram.tipo}</h5></td>
									<td><h5>${ram.capacidad}</h5></td>
									<td><h5>${ram.precio}€</h5></td>
									<td><h5>
											<a
												href="${pageContext.request.contextPath}/administracion/editaRam/${ram.id}.html">Edita</a>
										</h5></td>
								</tr>
							</c:forEach>
						</table>
